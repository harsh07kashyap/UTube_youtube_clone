  ...
  const fetchThumbnails = async () => {
    try {
      const response = await axios.get(
        `${backend_url}/api/homeContent/fetchthumbnails`
      );
  ...
  const onClick = async (videoId, currentViews) => {
    try {
      ...
      const response = await axios.put(
        `${backend_url}/api/homeContent/updateViews/${videoId}`,
        { views: newViews },
        {
          headers: {
            "auth-token": token,
      ...
      {images.map((image) => (
        <div key={image._id} className="card_own" style={{ width: "250px" }}>
          <img
            ...
            src={`${backend_url}/uploads/images/${image.filename}`}
            alt={`${backend_url}/uploads/images/${image.filename}`}
          ></img>
            ...
            <div className="card-body-one">
              {image.profilePicture ? (
                <img
                  ...
                  src={`${backend_url}/uploads/profiles/${image.profilePicture}`} // Fetch from server
                  alt="Profile"
                />
              ) : (
                <img
