...
const SubscribeButton = ({ userId}) => {
    ...
    useEffect(() => {
        const fetchSubscriptionStatus = async () => {
            try {
                ...
                const res = await axios.get(`http://localhost:5000/api/channel/check-subscription/${userId}`, config);
    ...
    const handleSubscribe = async () => {
        ...
        try {
            ...
            const res=await axios.post(`http://localhost:5000/api/channel/subscribe/${userId}`, {}, config);
