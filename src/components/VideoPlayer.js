  ...
  const { id } = useParams();
    ...
    useEffect(() => {
      ...
      const fetchcomments=async()=>{
        try{
          const response = await axios.get(`http://localhost:5000/api/videoplayer/comments/${id}`);
      ...
      const fetchLikeStatus=async()=>{
        try{
          ...
          const response = await fetch(`http://localhost:5000/api/videoplayer/like_status/${id}`,{
            method:'GET',
            // credentials: 'include',
            headers: {
    ...
    const toggleLike = async () => {
      try {
        const token=localStorage.getItem('token')
        const response = await fetch(`http://localhost:5000/api/videoplayer/likes/${id}`, {
          method: 'POST',
    ...
    const handleChange=async(e)=>{
      try{
        ...
        const token = localStorage.getItem('token');
        const response = await fetch(`http://localhost:5000/api/videoplayer/videos/${id}/comment`, {
      ...
      <ul className="suggested-video">
          <div className="card" style={{width: "25rem"}}>
            ...
            <div className="card-body">
              ...
              <button style={{ color: hasLiked ? 'blue' : 'gray' }}
                onClick={toggleLike}>
            ...
            <form onSubmit={handleChange}>
              <input type="text" onChange={onChange}  value={newcomment} id="comment" name="comment" />
