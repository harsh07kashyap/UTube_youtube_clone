...
const Profile = () => {
  ...
  const handleUpload = async (event) => {
    ...
    try {
      ...
      await axios.put("http://localhost:5000/api/profile/upload", formData, {
  ...
  const fetchVideoCount = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        "http://localhost:5000/api/profile/fetchvideocount",
        {
          headers: { "auth-token": token },
        }
      );
  ...
  const fetchProfilePics = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        "http://localhost:5000/api/profile/fetchprofilepic",
        {
          headers: { "auth-token": token },
        }
      );
